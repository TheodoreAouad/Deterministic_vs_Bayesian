...
class GaussianClassifierMNISTNoBatchNorm(GaussianClassifierMNIST):
    """
    Same as GaussianClassifierMNIST but without batch norm layers
    """
...
class GaussianClassifierCIFAR(nn.Module):
    ...
    def forward(self, x, determinist=None):
        if determinist is not None:
            ...
            is_forward_determinist = self.determinist
        output = self.pool1(F.relu(self.gaussian_conv1(x, determinist=is_forward_determinist)))
        output = self.pool2(F.relu(self.gaussian_conv2(output, determinist=is_forward_determinist)))
        output = output.view(-1, 32 * self.dim_input // 4 * self.dim_input // 4)
        output = F.softmax(self.gaussian_linear(output, determinist=is_forward_determinist), dim=1)
